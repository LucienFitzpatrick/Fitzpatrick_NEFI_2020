...
dat.npn$rmean <- rollmean(dat.npn$color.clean, k = 7, fill = NA)
...
"
#dat.npn <- dat.npn[dat.npn$week < 48,] 

data.new <- list(y = dat.npn$rmean, n = length(dat.npn$rmean), time = dat.npn$day_of_year-(min(dat.npn$day_of_year)-1), nt = 365-213, 
...
dat.2018 <- dat.2018[dat.2018$day_of_year > 29 & dat.2018$day_of_year< 48, ]
...
data_2018 <- list(y = dat.2018$color.clean, n = length(dat.2018$color.clean), time = dat.2018$day_of_year-(min(dat.2018$day_of_year)-1), nt = length(unique(dat.2018$day_of_year)), 
                  a_add=100, r_add=1, x_ic = -100, tau_ic = 1000)
...
Npnclike = matrix(0,nrow = Nmc, ncol = length(unique(dat.2018$day_of_year)))
# i=1
for(i in 1:Nmc){
  Npnlike[i,] = dbinom(dat.2018$color.clean,1,x.ip_LOD[i,dat.2018$day_of_year-30],log=TRUE)  ## calculate log likelihoods
  Npnlike[i,is.na(Npnlike[i,])] = 0       ## missing data as weight 1; log(1)=0
  tmp = tapply(Npnlike[i,],dat.2018$day_of_year-30,sum)
  tmp2 = rep(0, length(unique(dat.2018$day_of_year)))
...
plot(dat.2018$day_of_year, dat.2018$color.clean ,pch="+",cex=0.5, xlab = "day_of_year", ylab = "Fall Color", main = "Iterative 2018 Oak collection forecast")
ecoforecastR::ciEnvelope(time,ip_ci_LOD[1,],ip_ci_LOD[3,],col=col.alpha("purple",0.5))
for(i in seq_along(s)) {
