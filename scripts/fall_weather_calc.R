...
weather_calc <- function(met.all){
  ...
  met.all$CDD5 <- ifelse(met.all$TMEAN>5, met.all$TMEAN-5, 0)
  met.all$CDD0 <- ifelse(met.all$TMEAN>0, met.all$TMEAN, 0)
  ...
  for(YR in unique(met.all$year)){
    ...
    for(i in 1:nrow(dat.tmp)){
      ...
      if(is.na(dat.tmp$CDD0[i]) & d0.miss<=7){ #YOU CHANGED THIS TO 7 FOR NOW BUT CHANGE BACK
        ...
        cdd0.cum <- cdd5.cum+dat.tmp$CDD0[i] 
      }
